...
var imageCompiler = function (ref) {
    ...
    return (renderer.image = function (href, title, text) {
        var url = href//img相对markdown的路径
        var currMdPath =  router.getCurrentPath()//markdown文件路径
        var currPath = currMdPath.slice(0,currMdPath.lastIndexOf('/'))//舍弃文件名获取目录名
        console.log("curr:",currPath)
        var fullUrl = path.join(currPath,url)
        var temp = ""
        var imgUrl = path.join(contentBase,fullUrl)
        if (imgUrl.indexOf('http')>-1){
            let pm = fetch(imgUrl)
            pm.then(res=>{temp=res})
            sleep(1000*1)
        }else{
            temp = fs.readFileSync(imgUrl,"binary")
        }
        var buffer = Buffer.from(temp, 'binary');
        var r = new RegExp("\.(png|jpg|gif|jpeg|webp)$","i")
        var filetype = r.exec(fullUrl)[1]
        var imgB64 = 'data: image/'+ filetype +';base64,' + buffer.toString('base64');
        var attrs = [];
        ...
        if (attrs.length > 0) {
            return ("<img src=\"" + imgB64 + "\" data-origin=\"" + href + "\" alt=\"" + text + "\" " + (attrs.join(
                ' '
            )) + " />");
        }

        return ("<img src=\"" + imgB64 + "\" data-origin=\"" + href + "\" alt=\"" + text + "\"" + attrs + ">");
    });
};
...
var Renderer = function Renderer(ref) {
    ...
    this._renderHtml(
        'inject-config',
        ("<script>window.$docsify = " + (JSON.stringify(config2)) + "</script>")
    );
